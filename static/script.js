...
// Add a new room `name` to state object and change to it.
//Returns `true` if the room didn't already exist and false otherwise.
...
// Add `message` from `username` to `room`.
// If `push`, then actually store the message.
// If the current room is `room`, render the message.
...
function subscribe(uri) {
  ...
  function connect(uri) {
    // EventSource object is javascript interface to open persistent connection to HTTP server
    // Will stay open until event.close() is called
...
function init() {
  // Add some rooms to state for use in chatting
  addRoom("lobby");
  addRoom("rocket");
  changeRoom("lobby");
  addMessage(
  ...
  newMessageForm.addEventListener("submit", (e) => {
    ...
    const username = usernameField.value || "guest"; // Default username to guest
    if (!message) return;

    if (STATE.connected) {
      // If we are successfully connected, make a post request to our /message endpoint
      // containing the values parsed from the HTML form
      fetch("/message", {
        method: "POST",
        body: new URLSearchParams({ room, username, message }),
      }).then((response) => {
        if (response.ok) messageField.value = ""; // Clear the message field if message sent successfully
      });
    }
  });
